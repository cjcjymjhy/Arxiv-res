...
def filter_by_keywords(paper, keywords):
    """水文预报 AND 可解释AI 双条件筛选（扩充完整版，含流域水质）"""
    ...
    has_water = any(kw in text for kw in water_group)
    has_xai = any(kw in text for kw in xai_group)
    # 水文主题 + 可解释性，深度学习模型自动被包含在结果里
    return has_water and has_xai
...
def published_date_string(paper):
    """Return the arXiv API published date used for folders and tables."""
    published = paper.published
    if published.tzinfo is None:
        published = published.replace(tzinfo=ARXIV_TZ)
    return published.astimezone(ARXIV_TZ).strftime('%Y-%m-%d')


def arxiv_id_month(paper):
    """Return YYMM from IDs like 2608.25030v1."""
    match = re.match(r'(\d{4})\.', paper.get_short_id())
    return match.group(1) if match else None


def paper_month_matches_published_date(paper):
    """Keep papers whose arXiv ID month matches the published date month."""
    id_month = arxiv_id_month(paper)
    if id_month is None:
        return True
    return id_month == published_date_string(paper).replace('-', '')[2:6]


def water_priority_score(paper):
    """可解释性论文中，水文相关内容排前面。"""
    text = (paper.title + " " + paper.summary).lower()
    water_group = {
        "water",
    ...
    return sum(1 for kw in water_group if kw in text)
...
ARXIV_API_DELAY = 30.0      # 请求间隔（秒），保守策略降低请求压力
ARXIV_MAX_RETRIES = 4       # 429 错误最大重试次数
ARXIV_RETRY_BACKOFF = 180.0 # 429 错误初始退避时间（秒），每次翻倍
...
def fetch_papers_by_category(client, category, start_date, end_date, max_results, keywords):
    ...
    for attempt in range(ARXIV_MAX_RETRIES):
        try:
            ...
            papers.sort(key=water_priority_score, reverse=True)
            # 成功完成，跳出重试循环
            break
            ...
            if e.status == 429:
                if attempt < ARXIV_MAX_RETRIES - 1:
                    ...
                    raise RuntimeError(f"arXiv API rate limited category {category} after {ARXIV_MAX_RETRIES} attempts")
            else:
                raise RuntimeError(f"arXiv API failed for category {category}: HTTP {e.status}") from e
        except Exception as e:
            raise RuntimeError(f"Failed to fetch arXiv category {category}") from e
...
def generate_markdown(papers_by_category, config, existing_ids=None, last_update=None):
    ...
    for category, papers in papers_by_category.items():
        ...
        for paper in new_papers:
            if not paper_month_matches_published_date(paper):
                print(
                    f"  Skip date-mismatch paper: {paper.get_short_id()} "
                    f"published={published_date_string(paper)}"
                )
                continue
            total_new += 1
            date = published_date_string(paper)
...
def update_markdown_files_by_date(papers_by_date, config):
    ...
    for date in sorted(papers_by_date.keys(), reverse=True):
        ...
        existing_file_ids = get_existing_papers_in_file(file_path)
        ...
        for category in sorted(papers_by_category.keys()):
            ...
            papers = [paper for paper in papers if paper.get_short_id() not in existing_file_ids]
            if not papers:
                continue
            existing_file_ids.update(paper.get_short_id() for paper in papers)
            ...
            if include_summary:
                ...
                for paper in papers:
                    ...
                    published_date = published_date_string(paper)
        ...
        action = "追加文件" if file_exists else "创建文件"
        print(f"  {action}: {file_path} ({len(papers_by_category)} 个分类)")
...
def get_existing_papers_in_file(file_path):
    """提取单个日期文件里已经存在的论文 ID。"""
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    return set(re.findall(r'arxiv\.org/abs/(\d+\.\d+)', content))
...
def save_state(state_file, latest_paper_date):
    ...
    # 重复论文通过论文 ID 去重来避免
    if latest_paper_date.tzinfo is None:
        latest_paper_date = latest_paper_date.replace(tzinfo=ARXIV_TZ)
    paper_date = latest_paper_date.astimezone(ARXIV_TZ).strftime('%Y-%m-%d')
...
def save_last_update_time(update_time):
    ...
    if update_time.tzinfo is None:
        update_time = update_time.replace(tzinfo=ARXIV_TZ)
...
def main():
    ...
    if state is None:
        ...
        start_date = datetime.strptime(config['start_date'], '%Y-%m-%d').replace(tzinfo=ARXIV_TZ)
        ...
        start_date = datetime.strptime(last_paper_date_str, '%Y-%m-%d').replace(tzinfo=ARXIV_TZ)
        print(f"从最新论文发布日期 {start_date.strftime('%Y-%m-%d')} 开始爬取")
    
    end_date = datetime.now(ARXIV_TZ)
    ...
    client = arxiv.Client(
        page_size=max_results,   # 每页结果数，与每分类抓取数量一致，减少请求量
        delay_seconds=ARXIV_API_DELAY,  # 请求间隔 30 秒，保守策略
    ...
    if new_count > 0:
        ...
        print(f"OK created {files_created} files with {total_papers} new papers in {docs_dir}")
    else:
        print("OK no new papers to add")
    ...
    print(f"\nDone. Crawled {new_count} new papers")
